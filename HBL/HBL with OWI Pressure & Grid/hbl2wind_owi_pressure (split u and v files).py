...
class OwiAscii:
    ...
    def __init__(self, u_p1_filename, v_p1_filename, u_p2_filename, v_p2_filename, pre_filename, idx):
        self.__u_p1_filename = u_p1_filename
        self.__v_p1_filename = v_p1_filename
        self.__u_p2_filename = u_p2_filename
        self.__v_p2_filename = v_p2_filename
    ...
    def get(self):
        ...
        if self.__idx < 490:
            ...
            f_u = Dataset(self.__u_p1_filename, 'r')
            f_v = Dataset(self.__v_p1_filename, 'r')
        elif self.__idx >= 490:
            hbl_idx = (self.__idx-490) * 15 + 4 #p1 has 1601 time steps and 1605 is the next time step that corresponds to an OWI point
            f_u = Dataset(self.__u_p2_filename, 'r')
            f_v = Dataset(self.__v_p2_filename, 'r')
...
def main():
    ...
    # Arguments
    parser.add_argument("files", metavar="file", type=str, help="Files to be converted; must be exactly five with p1 ""u"" file listed first, p1 ""v"" file listed second, p2 ""u"" file listed third, p2 ""v"" file listed fourth, and OWI ASCII PRE file listed fifth", nargs='+')
    ...
    while time_index - 384 < num_times: #This, plus making our data file class time-slice specific, lets us maintain the old OwiNetcdf class granularity and diverge less from the original code
        owi_ascii = OwiAscii(file_list[0], file_list[1], file_list[2], file_list[3], file_list[4], time_index)

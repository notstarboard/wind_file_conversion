...
class OwiAscii:
    ...
    def get(self, idx):
        ...
        return WindData(self.__date, self.__grid, None, uvel, vvel)
...
def main():
    ...
    parser.add_argument("files", metavar="file", type=str,
                        help="Files to be converted; must be exactly two with ""pre"" file listed first", nargs='+')
    ...
    if num_files != 1:
        raise RuntimeError("Must specify exactly one file (wind only)")
    ...
    while time_index < num_times:  # This, plus making OwiAscii time-slice specific, lets us maintain the old OwiNetcdf class granularity and diverge less from the original code
        owi_ascii = OwiAscii(file_list[0], time_index)
